...
check_dependencies() {
    ...
    if ! command -v nft >/dev/null 2>&1; then
        log ERROR "nftables is not installed. Install it with: apt install nftables"
        exit 1
    fi
    
    if ! command -v curl >/dev/null 2>&1; then
        log ERROR "curl is not installed. Install it with: apt install curl"
        exit 1
    fi
...
load_ipv4_addresses() {
    ...
    if [ ${#IP_ARRAY[@]} -eq 0 ]; then
        log WARN "No valid IPv4 addresses found"
        return 0
    fi
    ...
    for ip in "${IP_ARRAY[@]}"; do
        if $NFT add element inet $TABLE $SET_IPV4 "{ $ip }" 2>/dev/null; then
            ...
            if [ $((loaded % 100)) -eq 0 ]; then
                log INFO "Loaded $loaded IPv4 addresses..."
            fi
        else
            ((failed++))
        fi
    ...
    log SUCCESS "Loaded $loaded IPv4 addresses (failed: $failed)"
    echo "$loaded"
}
...
load_ipv6_addresses() {
    ...
    if [ ! -f "$TOR_LIST_IPV6_FILE" ]; then
        log WARN "IPv6 list file not found"
        echo "0"
        return 0
    fi
    
    log INFO "Loading IPv6 addresses..."
    
    readarray -t IP_ARRAY < <(grep -Eo '([0-9a-fA-F:]+)' "$TOR_LIST_IPV6_FILE" | sort -u)
    
    if [ ${#IP_ARRAY[@]} -eq 0 ]; then
        log WARN "No valid IPv6 addresses found"
        echo "0"
        return 0
    fi
    ...
    for ip in "${IP_ARRAY[@]}"; do
        if $NFT add element inet $TABLE $SET_IPV6 "{ $ip }" 2>/dev/null; then
            ...
            if [ $((loaded % 50)) -eq 0 ]; then
                log INFO "Loaded $loaded IPv6 addresses..."
            fi
        else
            ((failed++))
        fi
    ...
    log SUCCESS "Loaded $loaded IPv6 addresses (failed: $failed)"
    echo "$loaded"
}
